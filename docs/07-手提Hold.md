...
> **2026-09-05：** `POST /admin/hold/api/manual_hold` **已恢复**。创建页、`GET /admin/hold/api/manual_hold/products` / `recent`、以及 `GET /admin/hold/api/annex_image` / `annex_zip` 仍返回 **HTTP 410**。数据分析用 testlog FTP 探活 `GET /api/common_data/ftp/status` **仍可用**。
