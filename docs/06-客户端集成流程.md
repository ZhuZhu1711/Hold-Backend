...
3. 用户选中一条记录做数据分析（可选但推荐）
   GET /admin/hold/api/analysis
     ...
     &record_type={RECORD_TYPE}&station={STATION}
...
flowchart TD
  ...
  list --> analyze[GET_analysis]
  analyze --> fvi{RECORD_TYPE_1?}
  fvi -->|yes| fviApi[GET_fvi_defect_details]
  fvi -->|no| canDispose{CAN_DISPOSE?}
  fviApi --> canDispose
...
| 模块 | 依赖接口 | 要点 |
| --- | --- | --- |
| 登录 | `/api/login` | 密码先 MD5 再提交；非工程师账号拒绝进入主界面 |
| 待办列表 | `/eng/api/holding_records` | 默认 `pending_only=1`；提供「全部所属」开关时去掉该参数 |
| 分析面板 | `/admin/hold/api/analysis` + 可选 FVI | bysite 失败勿阻断处置按钮 |
