...
1. 本地 MD5(明文密码) → POST /api/login（body.password 为 32 位 hex，勿传明文）
   └─ 校验 data.role === 1；若 data.must_change_password 为 true，提示先到 Web 后台改密，不要进主界面
...
3. 用户选中一条记录做数据分析（可选但推荐）
   ...
   - HOLD_CODE 含 AQL_HOLD → 不调 analysis，改调：
      GET /admin/hold/api/annex_image?record_id={ID}&index=0,1,...
      ANNEX_COUNT=0 时不显示图片
...
| 模块 | 依赖接口 | 要点 |
| --- | --- | --- |
| 登录 | `/api/login` | 密码先 MD5 再提交；非工程师账号拒绝进入主界面；`must_change_password` 时去 Web 改密；登录后探活 FTP |
