...
@login_required
def api_annex_image():
    """按 hold_record 下载附件图。附件 FTP 下载已关闭。"""
    return manual_hold_ctrl.gone_response(manual_hold_ctrl.ANNEX_FTP_TAKEN_DOWN_MSG)


@hold_report_bp.route('/api/annex_zip', methods=['GET'])
@login_required
def api_annex_zip():
    """打包下载该 hold_record 全部附件。附件 FTP 下载已关闭。"""
    return manual_hold_ctrl.gone_response(manual_hold_ctrl.ANNEX_FTP_TAKEN_DOWN_MSG)
